# 1. 将以下内容保存为一个 .ps1 脚本文件，比如 kitti_download.ps1
# 2. 在当前目录下用 PowerShell 执行： .\kitti_download.ps1
...
foreach ($file in $files) {
    ...
    if ($file -like '*.zip') {
        ...
        # 如果不是zip，则拼接成 drive_xxxx_sync.zip 这样的形式
        $shortname = "${file}_sync.zip"
        $fullname  = "${file}/${file}_sync.zip"
    }

    Write-Host "`n正在下载: $shortname"

    # 构造下载链接
    $url = "https://s3.eu-central-1.amazonaws.com/avg-kitti/raw_data/$fullname"

    # 下载文件
    Invoke-WebRequest -Uri $url -OutFile $shortname

    Write-Host "解压中: $shortname"
    # 解压文件到当前目录
    Expand-Archive -Path $shortname -DestinationPath "."

    # 删除下载的zip包
    Remove-Item -Path $shortname -Force

    Write-Host "完成: $shortname"
}
